...
const Chat = () => {
  ...
  return (
    <div className="fixed bottom-0 right-0 z-[9999] flex flex-col items-end font-jost pointer-events-none p-4 sm:p-5">
      
      {/* 1. PREVIEW BUBBLE (Apple Music Style) */}
      <AnimatePresence>
        {showPreview && !isOpen && (
            <motion.div
            ...
            >
                ...
                <div className="flex flex-col overflow-hidden">
                    <span className="text-xs font-bold text-white leading-tight">DanZ-Kev Bot</span>
                    <span className="text-[10px] text-white/40 leading-tight mt-0.5 whitespace-nowrap">Rekomendasi lagu? 🎵</span>
                </div>
      ...
      {/* 2. CHAT WINDOW (Apple Music Dark Aesthetic) */}
      <AnimatePresence>
        {isOpen && (
          <motion.div
            ...
            className="pointer-events-auto fixed inset-0 z-[10000] bg-[#0a0a0a] flex flex-col sm:absolute sm:inset-auto sm:bottom-[70px] sm:right-0 sm:w-[380px] sm:h-[580px] sm:rounded-3xl sm:shadow-[0_25px_80px_rgba(0,0,0,0.6)] sm:border sm:border-white/10 overflow-hidden"
          >
            ...
            <div className="relative bg-[#1c1c1e]/80 backdrop-blur-2xl px-5 py-4 flex justify-between items-center shrink-0 border-b border-white/5">
              ...
              <button onClick={() => setIsOpen(false)} className="w-8 h-8 flex items-center justify-center rounded-full bg-white/10 hover:bg-white/20 transition-all active:scale-90">
      ...
      {/* 3. FAB BUTTON (Apple Music Style) */}
      <AnimatePresence>
        {!isOpen && (
            <motion.button
                ...
                whileTap={{ scale: 0.9 }}
                onClick={() => setIsOpen(true)}
                className="pointer-events-auto w-14 h-14 bg-[#1c1c1e]/90 backdrop-blur-xl text-white rounded-full shadow-[0_8px_30px_rgba(0,0,0,0.3)] flex items-center justify-center border border-white/10 hover:bg-[#2c2c2e] transition-all z-[9990]"
            >
                <FontAwesomeIcon icon={faCommentDots} className="text-xl text-white/80" />
            </motion.button>
        )}
      </AnimatePresence>
