    ...
    return (
        ...
        >
            <div className="flex items-stretch gap-3">
                {/* Apple Music style left indicator bar */}
                <div className={`w-[3px] shrink-0 rounded-full transition-all duration-500 ${
                    isActive ? "bg-white/90" : "bg-transparent"
                }`} />
                <p className={`font-extrabold text-[26px] leading-snug text-white tracking-tight ${isActive ? "active-lyric-glow-text" : ""}`}>{line.text}</p>
            </div>
        </div>
    );
...
// --- MAIN CARD ---
const Card = ({ fullScreen = false, onOpenChat }) => {
    ...
    useEffect(() => {
        ...
        getLocalMetadata(currentTrack).then(async (data) => {
            ...
            let karaokeUrl = null;
            if (data.karaokeUrl) {
                karaokeUrl = await fetchAudioBlob(data.karaokeUrl);
                setKaraokeBlobUrl(karaokeUrl);
            } else {
            ...
            if (instruRef.current) {
                if (karaokeUrl) { 
                    instruRef.current.src = karaokeUrl; 
                    ...
                    {(!fullScreen || (showLyrics && !isDesktop) || showPlaylist) && (
                        <div className="flex items-center space-x-5 shrink-0 mb-4 relative z-50 animate-fade-in">
                            ...
                            <div className="min-w-0 flex flex-col justify-center flex-1">
                                <h3 className="font-bold text-white truncate text-[18px] drop-shadow-md">{result.title}</h3>
                                <p className="text-white/60 font-medium text-[13px] truncate drop-shadow-sm">{result.artist}</p>
                            </div>
                            <button onClick={() => { setShowLyrics(!showLyrics); setShowPlaylist(false); }} className={`w-10 h-10 rounded-full flex items-center justify-center transition-all active:scale-90 ${showLyrics && !showPlaylist ? "bg-white text-black" : "bg-white/10 text-white hover:bg-white/20"}`}>
                             ...
                             >
                                ...
                                >
                                     ...
                                     <div className="absolute inset-0 bg-cover bg-center rounded-2xl bg-gray-800"
                                          style={{ 
                                              backgroundImage: result.cover ? `url(${result.cover})` : 'none',
                                              filter: 'blur(50px)',
                                              opacity: 0.5,
                                              transform: 'scale(1.1)'
                                          }}></div>
                                     ...
                                     {/* Main Image */}
                                     <div className="relative w-full h-full rounded-2xl shadow-[0_20px_60px_rgba(0,0,0,0.6)] overflow-hidden border border-white/10 bg-[#111]">
                                         {result.cover ? <img src={result.cover} alt="Cover" className="w-full h-full object-cover" /> : <div className="w-full h-full flex items-center justify-center"><FontAwesomeIcon icon={faApple} className="text-white/20 text-6xl" /></div>}
                                     </div>
                                </div>
